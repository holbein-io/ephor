...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      <nav className="sticky top-0 z-50 h-14 bg-bg-secondary border-b border-border animate-slide-in">
        <div className="app-shell flex items-center h-full px-6">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="flex items-center gap-2 bg-bg-tertiary border border-border rounded-lg px-3 py-1.5 w-[200px] focus-within:border-accent focus-within:w-[280px] transition-all duration-200">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="bg-transparent border-none outline-none font-body text-sm text-text-primary w-full placeholder:text-text-tertiary"
              />
              <kbd className="font-mono text-micro text-text-tertiary bg-bg-hover px-1.5 py-0.5 rounded border border-border ml-auto flex-shrink-0">
                /
              </kbd>
            </div>
      ...
      <main className="app-shell px-6 py-7">
        {children}
      </main>
    </div>
  );
